module ddr_ctrl(
	...
	localparam	tRCD=9,tCWL=11,tCL=10,tRP=8;
	...
	// dq_i = dq를 이용하여 dq에서 값을 뽑아 reg에 대입할 수 있게 설계한 것 같음.
	assign dq = dq_oe ? dq_o: 'bZ;
	assign dq_i = dq;

	// FSM 관련 State 이동. FSM에는 Data 이동 같은 Code를 삽입하지 말고, 그건 따로 처리할 것.
	always @(posedge clk, negedge rstn)
	if	(!rstn)	curr_state	<= 0;	// FSM에서는 State만 조절하는 방식으로 설계해야 함.

	else begin
		case (curr_state)
		INIT: curr_state <= MRS;
		MRS: if (cnt == 1) curr_state <= IDLE;
		IDLE : if (i_req & o_ack) curr_state <= ACT;	// i_req를 입력 받으면 작동 가능하다는 신호를 출력하고, 그와 동시에 ACT로 상태를 이동시켜야 함.
		ACT: begin
			if (cnt == tRCD) curr_state <= i_write ? WR: RD;	// 이런 방식으로 조건문을 우변에 넣어서 간단하게 표현할 수 있음.
		end
		WR : begin
			if (cnt == tCWL+4+1) begin	// CLK Timing에 유의 
				if (curr_row == next_row) curr_state <= i_write ? WR: RD;	// BANK 및 Array가 4개 있으니 이러한 구성을 채택함. 근데 이거 수정 들어가야할 듯.
				else curr_state <= PRE;
			end
		end
		RD : begin
			if (cnt == tCL+4+1) begin	// 역시나 CLK Timing에 유의
				if (curr_row == next_row) curr_state <= i_write ? WR: RD;	// WR / RD를 바로 이동시킬 수 있도록 작성했는데... 이건 WR, RD를 하나로 묶어서 처리해도 될 듯 하긴 함.		
				else curr_state <= PRE;
			end
		end
		PRE: begin
			if (cnt === tRP) curr_state <= IDLE;	// tRP 시간 지난 후 IDLE로 이동해야 하므로.
		end
		endcase
	end

	always @(posedge clk, negedge rstn)	// cnt의 초기화를 이런 방식으로 처리함.
	if	(!rstn)	cnt	<= 0;	
	else begin
		case (curr_state)
	...
	assign	o_ack = (curr_state == IDLE) || // o_ack의 출력 방식 또한 이렇게 따로 처리.
