...
var.add_argument("--epoch_start", type=int, default=15, help="모델을 불러오고 싶은 경우 미리 훈련된 모델의 epoch 수 지정")
...
optimizer = Adam(itertools.chain(em.parameters(), ed.parameters(),  cc.parameters(), cd.parameters(), gn.parameters()), lr=1e-6, eps=1e-8)
...
def main():
    ...
    for epoch in range(n_epoch):
        for batch_idx, batch in enumerate(ds_loader):
            ...
            optimizer.zero_grad()
            pred = create_img(input_img).to(device)
            ...
            loss = c_loss + 8*similar_loss + 10*s_loss

            loss.backward()
            optimizer.step()


            sys.stdout.write(f'\repoch : {epoch+s_epoch}, batch : {batch_idx}/{len(ds_loader)}, loss : {loss.item():.5f}, c_loss : {c_loss.item():.5f},  grad_loss : {s_loss.item():.5f}, ssim : {similar_loss.item():.5f}')
            sys.stdout.flush()
        ...
        if (epoch+s_epoch) % 5 == 0:
            ...
            save_path = f"./saved_models/tone_curve_ckpt_{epoch}.ckpt"

            torch.save(checkpoint, save_path)
            sys.stdout.write(f'{epoch} Checkpoint 저장 완료 !')
            sys.stdout.flush()
